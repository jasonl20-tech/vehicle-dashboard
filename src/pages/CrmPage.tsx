...
function KvPresence({ kvKey }: { kvKey: string | null }) {
  const has = Boolean(kvKey?.trim());
  return (
    <span
      className={
        has
          ? "inline-flex rounded-full border border-emerald-200/90 bg-emerald-50 px-2 py-0.5 text-[11px] font-medium text-emerald-900"
          : "inline-flex rounded-full border border-hair bg-ink-50/80 px-2 py-0.5 text-[11px] text-ink-500"
      }
    >
      {has ? "Vorhanden" : "Nicht vorhanden"}
    </span>
  );
}
...
export default function CrmPage() {
  ...
  /** ID & Aktualisiert: standardmäßig aus, optional am Tabellenende einblenden */
  const [showIdAndUpdated, setShowIdAndUpdated] = useState(false);
  ...
  const colCount = 7 + (showIdAndUpdated ? 2 : 0);

  const goDetail = (id: string) => {
    navigate(`/kunden/crm/${encodeURIComponent(id)}`);
  };
      ...
      <PageHeader
        ...
        description={
          <span>
            ...
            Tabelle <code className="text-[12.5px]">crm_customers</code>. Zeile
            anklicken für die vollständige Ansicht. Die Suche durchsucht weiterhin
            alle Spalten (inkl. ID und KV-Key), auch wenn diese in der Tabelle nicht
            sichtbar sind.
          </span>
        }
      ...
      >
        <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-lg border border-hair/90 bg-paper/70 shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-hair/80 bg-paper/90 px-3 py-2.5 sm:px-4">
            <div className="relative min-w-0 max-w-md flex-1">
              ...
              <input
                ...
                placeholder="Suche in allen Feldern (E-Mail, Firma, Notizen, Status, ID, KV-Key …)"
                value={qIn}
            ...
            <div className="flex flex-wrap items-center gap-3">
              <label className="inline-flex cursor-pointer items-center gap-2 text-[12px] text-ink-600 select-none">
                <input
                  type="checkbox"
                  className="rounded border-hair text-ink-800 focus:ring-ink-400"
                  checked={showIdAndUpdated}
                  onChange={(e) => setShowIdAndUpdated(e.target.checked)}
                />
                ID &amp; Aktualisiert
              </label>
              <button
          ...
          <div className="min-h-0 flex-1 overflow-x-auto overflow-y-auto">
            ...
            <table className="w-full min-w-[min(100%,920px)] border-collapse text-left">
              <thead>
                <tr>
                  ...
                  <th className={TH}>Key-Zuordnung</th>
                ...
                {rows.map((r: CrmCustomerRow) => {
                  ...
                  return (
                    <tr
                      ...
                      role="link"
                      tabIndex={0}
                      className="cursor-pointer border-b border-transparent hover:bg-ink-50/80 focus:bg-ink-50/80 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-ink-300"
                    ...
                    >
                      ...
                      <td className={TD}>
                        <KvPresence kvKey={r.kv_key} />
                      </td>
                      <td className={TD + " break-words max-w-[200px] text-ink-600"}>
                      ...
                      {showIdAndUpdated && (
                        <>
                          <td
                            className={TD + " max-w-[120px] break-all font-mono text-[10px] text-ink-500"}
                          >
                            {r.id}
                          </td>
