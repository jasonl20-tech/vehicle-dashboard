...
/**
 * Editor für einen einzelnen Kunden-Key (oder Test-Key).
 *
 * Lädt den Wert aus dem `useJsonApi`-Result, parst ihn in ein bearbeitbares
 * Modell und ruft `putCustomerKey` beim Speichern. Wird sowohl von der
 * dedizierten Detail-Seite als auch von der Split-View-Listen-Seite genutzt.
 */
export default function CustomerKeyEditor({
  kid,
  one,
  err,
  loading,
  onAfterSave,
  containerClassName = "max-w-4xl border-t border-hair pt-8",
  showHeader = true,
}: {
  kid: string;
  ...
  loading: boolean;
  onAfterSave: () => void;
  /** Container-Klassen — anpassbar für Split-View (kein border-t etc.). */
  containerClassName?: string;
  /** Header (Plan-Eyebrow + Customer-Label + Key-ID) anzeigen? */
  showHeader?: boolean;
}) {
        ...
        <div className="space-y-5">
          <Grid2>
            <Field label="status">
              ...
              >
                {[...new Set([model.status, ...CUSTOMER_KEY_STATUSES])].map(
                  (s) => (
            ...
            <Field
              label="plan_id"
              hint="„test“ in plan_id/plan_name → Kundentest keys"
            >
              <TextInput
                value={model.plan_id}
                onChange={(v) =>
